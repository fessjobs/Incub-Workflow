...
| Stammdaten-Import | `src/lib/einsatz/import/` – `parse-table.ts` (CSV/XLSX, Formaterkennung, Spaltenerkennung), `pdf-tabelle.ts` (PDF-Listen über die Claude API), `stammdaten.ts` (Vorschau + Import für Mitarbeiter und Kunden) |
...
`/einsaetze/personal/import` und `/einsaetze/kunden/import` (Rolle Dispo, also auch Disponent) lesen Excel-, CSV- **und PDF-Listen**. Die Spalten werden an den Überschriften erkannt, die Reihenfolge spielt keine Rolle; Trennzeichen (`;`, `,`, Tab, `|`) und Zeichensatz werden ermittelt.

| Format | Weg |
|---|---|
| CSV, TSV, TXT | lokal gelesen (Trennzeichen und Zeichensatz erkannt) |
| Excel (`xlsx`, `xlsm`) | lokal über `exceljs` |
| **PDF** | als `document`-Block an die Claude API, strukturierte Ausgabe → dieselbe `Tabelle` |

PDFs erkennt `isPdf()` an der Endung **und** an der Signatur `%PDF-`, eine falsch benannte Datei fällt also nicht durch. Das Modell (`ANTHROPIC_PARSER_MODEL`, dieselbe Einstellung wie der Einsatz-Parser) gibt Kopfzeile und Zeilen zurück; `pdf-tabelle.ts` bringt krumme Zeilen auf die Kopfbreite, trimmt und wirft leere raus. Ab da läuft alles wie bei CSV: Spaltenerkennung, Vorschau, Bestätigung. Ohne `ANTHROPIC_API_KEY` nennt die Fehlermeldung den Schlüssel **und** die Alternative (Excel/CSV); die Oberfläche sagt das schon vorher. Das Anthropic-Modul wird dynamisch geladen, damit ein CSV-Import die Abhängigkeit nicht mitzieht.
...
### 6a2. Ein Link je Schicht

Läuft ein Einsatz über mehrere Tage, hat oft jede Schicht ihre eigene Gruppe – der Aufbau am Donnerstag ist nicht die Crew vom Abbau am Samstag. Deshalb hat **jede Schicht ihren eigenen Gruppenlink** (`Shift.crewToken`, Migration `20260926090000_schicht_crewtoken`), derselbe Pfad `/e/crew/[token]`. In der Detailansicht steht unter dem Link für den ganzen Einsatz der Abschnitt **„Link je Schicht"** – je Schicht Datum, Zeit, Zähler „x von y unterschrieben" und dieselben vier Knöpfe (Nachricht kopieren, in WhatsApp öffnen, nur den Link, öffnen).

Die Nachricht ist auf die Schicht zugeschnitten: Kopfzeile mit Schichtnamen, nur diese eine Zeile mit Zeit und Treffpunkt, der Tag der Schicht statt der Spanne des Einsatzes, und der Satz „der Link gilt nur für diese Schicht".

Wer ihn öffnet, sieht **ausschließlich die Personen dieser Schicht** (`loadCrewByToken` löst erst den Einsatz-Token auf, dann den Schicht-Token, und kürzt `a.shifts` auf die eine Schicht). Oben steht ein Hinweis, für welche Schicht der Link gilt. Namen korrigieren, Personen ergänzen, Zeiten für alle übernehmen und unterschreiben funktionieren wie gewohnt.

Zwei Grenzen:

- **Zugriff:** `nurSchichtId` ist die Grenze jeder Schreibaktion. Wer über den Link der Aufbau-Schicht eine Person der Abbau-Schicht einreichen will, bekommt 403 („Person gehört nicht zu dieser Schicht.").
- **Kundenbestätigung:** Der Kunde bestätigt immer den **ganzen** Stundennachweis. Über einen Schichtlink geht das nur, wenn der Einsatz aus dieser einen Schicht besteht; sonst steht dort im Klartext, dass die Bestätigung über den Link für den gesamten Einsatz läuft. Das fertige PDF ist über beide Linkarten abrufbar und teilbar.

„Links erneuern" erneuert auch die Schichtlinks (abgelaufene oder fehlende, z. B. bei Schichten aus der Zeit vor dieser Änderung – die Migration hat Bestandsschichten aber schon Tokens gegeben).
...
- `npm run build && npm run test:e2e` – Playwright gegen den Standalone-Build:
  ...
  - `import.spec.ts`: Mitarbeiter- und Kundenimport mit Vorschau, Übernahme und Dublettenschutz beim zweiten Lauf; dazu der PDF-Weg – das Dateifeld nimmt PDFs an, und ohne API-Schlüssel steht die Ursache samt Alternative im Klartext (der Testserver läuft bewusst ohne Schlüssel). Der Erfolgsfall des PDF-Imports hängt an einem API-Aufruf und wird deshalb im Integrationstest mit gemocktem SDK geprüft, nicht im E2E.
...
19. **PDF-Import der Stammdaten braucht die Claude API.** Ein PDF ist kein zeilenweise lesbarer Text; eine lokale PDF-Textextraktion wäre bei eingescannten Listen wertlos und bei mehrspaltigen Layouts unzuverlässig. Deshalb liest das Modell die Tabelle – und deshalb verlangt die Vorschau weiterhin eine Bestätigung je Zeile, bevor etwas gespeichert wird.
20. **Anhänge**: höchstens 5 Dateien à 5 MB. Bilder und PDFs gehen an die Claude API, Text- und Tabellendateien werden lokal gelesen. Word-Dateien werden nicht unterstützt (kein Parser im Repo, und ein Screenshot davon tut es auch).
21. **Import**: Abgleich über Personalnummer, sonst exakter Name (keine Fuzzy-Suche) – ein falscher Treffer wäre hier teurer als ein Duplikat, das die Dispo sieht.
22. **Je ein PDF pro Art und Einsatz**: die ersetzte Fassung wird gelöscht, ihr SHA-256 bleibt im Audit-Log. Wer jede Zwischenfassung aufbewahren muss, setzt `replaceForAssignmentId` in `documents.ts` außer Kraft.
23. **Die Kundenbestätigung ist die Grenze** für Kopfdaten, Schichtzeiten und Besetzung – nicht die Konkretisierung. Das PDF lässt sich jederzeit neu erzeugen, die Unterschrift des Kunden nicht.
24. **Namen sind immer änderbar**, auch nach der Bestätigung und auch nach der Unterschrift der Person. Die Alternative – Namen einfrieren – wäre in der Praxis falsch (Heirat, Schreibfehler, Namenszusätze). Die Rückverfolgbarkeit hängt am Audit-Log und an der Prüfsumme der ersetzten PDF-Fassung, nicht an der Unveränderlichkeit des Stammsatzes.
25. **Umbenennen hängt um statt zu duplizieren**: Gibt es den neuen Namen schon im Stamm, wird die Einteilung dorthin verschoben. Nur ein Datensatz ohne Personalnummer, Kontaktdaten und zweite Einteilung wird umbenannt – sonst würde ein Tippfehler bei einer Person deren gesamte Historie umbenennen.
26. **Die Zeitvorgabe füllt nur vor, sie erfasst nicht**: „Zeiten für alle übernehmen“ legt keine Einträge für andere an. Jede Person prüft und unterschreibt selbst – ein Stundennachweis mit fremdbestimmten Zeiten ohne eigene Unterschrift wäre als Nachweis wertlos. Sie gilt je Schicht, nicht je Einsatz, und erst ab der ersten unterschriebenen Erfassung.
27. **Die Kurzanleitung wird pro Browser gemerkt, nicht pro Person**: Wer den Link regelmäßig bekommt, sieht sie einmal. Auf einem geteilten Crew-Gerät bekommt sie damit nur die erste Person zu sehen – dafür ist sie über das ? erreichbar, und die Schritte stehen ohnehin im Formular.
28. **Geleistet = unterschrieben**: Der Erfahrungszähler steigt mit der eigenen Unterschrift der Person, nicht mit der Freigabe und nicht mit der Einteilung. Wer eingeteilt war, aber nicht erschienen ist, taucht damit nicht als erfahren auf.
29. **Beurteilung je Einsatz statt je Person**: Eine Bewertung hängt an genau einer Schicht, nicht am Stammsatz. Die Personalübersicht rechnet daraus die Bilanz. So bleibt nachvollziehbar, wann und wofür jemand wie beurteilt wurde – und eine einzelne schlechte Schicht überschreibt nicht das Gesamtbild.
30. **Der Hinweis bei der Einteilung blockiert nicht**: Überwiegt Negatives, steht das rot bei der Person – die Dispo entscheidet trotzdem selbst. Eine harte Sperre wäre in der Praxis (kurzfristige Besetzung, wenig Auswahl) mehr im Weg als hilfreich.
31. **Erfahrungsstufen** (neu / eingearbeitet ab 1 / geübt ab 8 / erfahren ab 25) sind eine Annahme. Die nackte Zahl steht immer daneben; die Schwellen liegen in `STUFEN` in `service/personal.ts` und sind in einer Zeile geändert.
32. **Beurteilungen sind Beschäftigtendaten.** Sie liegen zwar nur im Backend, unterliegen aber der DSGVO: Betroffene haben ein Auskunftsrecht (Art. 15) auch auf interne Notizen. Deshalb sollten die Notizen sachlich und nachvollziehbar formuliert sein („zweimal unentschuldigt zu spät“ statt einer Charakterbeurteilung). Löschfristen sind noch festzulegen – siehe offener Punkt 9.
33. **Löschen heißt löschen**: kein Soft-Delete-Flag, das die Daten still weiterschleppt. Die Nachvollziehbarkeit hängt am Audit-Log, das einen vollständigen Abzug samt PDF-Prüfsummen behält – nicht daran, dass Zeilen in der Datenbank liegen bleiben.
34. **Die Freigabe ist die Grenze, nicht die Rolle**: Auch der Admin kommt an freigegebene Zeiten und gesperrte Monate nicht heran. Wer wirklich muss, nimmt erst die Freigabe zurück bzw. entsperrt den Monat – das ist dann eine eigene, protokollierte Handlung.
35. **Personen mit Unterschrift werden nicht gelöscht, sondern inaktiv gesetzt.** Ein unterschriebener Stundennachweis benennt die Person; verschwindet der Stammsatz, steht der Nachweis ohne Bezug da.
36. **Eingefügte Namen werden nicht automatisch übernommen**: Die Vorschau verlangt je Zeile eine Entscheidung (Treffer, Vorschlag, neu anlegen, überspringen). Ein falsch zugeordneter Name landet sonst in der Konkretisierung und im Lohnexport.
